...
class Class_WooCommerce_Views {
	...
	public function __construct() {
		...
		add_action('wp_ajax_wcviewsgui_wpv_woo_display_tabs', 'wcviewsgui_wpv_woo_display_tabs_func');
		...
		//WooCommerce Views 2.5.9+ Filter shortcode usage for general usability
		add_filter('wcviews_filter_shortcode_usage',array($this,'wcviews_allow_loop_related_shortcodes_only'),10,1);
...
<?php 
	...
	public function check_if_php_template_contains_wp_loop($template,$is_using_genesis=false) {
		...
		} elseif ( 'single-product.php' == basename($template)) {
	...
	**/
	...
	public function adjust_wc_views_image_placeholder($imagehtml,$postid) {
	     ...
	     * on single products
	     */
	    $is_product_listing = $this->wcviews_is_woocommerce_listing();
	    $doing_views_loop = false;
	    if ( defined('WCVIEWS_DOING_LOOP') ) {
	    	if ( true === WCVIEWS_DOING_LOOP ) {
	    		$doing_views_loop = true;
	    	}
	    }
	    if ( ( true === $is_product_listing ) || ( true === $doing_views_loop ) ) {
	    	//Filter
	...
	*/
	...
	public function wpv_woo_add_shortcode_in_views_popup_aux($items) {		
		...
		$items['WooCommerce']['productwoodisplayingtabs'] = array(
				...
				'wpv-woo-display-tabs',
				'Basic',
				'wcviews_insert_wpv_woo_display_tabs(); return false;'
		);
		//[wpv-woo-onsale]
   ...
   <?php
	 ...
	 * A call to this method should set the property 'wcviews_shortcode_executed' to TRUE.
  	...
  	public function wcviews_allow_loop_related_shortcodes_only($the_items) {
  		...
  		$run_filter=false;
  		if ((isset($_GET['page'])) && (is_admin()) && (isset($_GET['view_id']))) {  				
  			...
  			//Doing ajax such as using the Loop wizard functionality
  			...
  			//Check for cases where its used inside Layouts
  			if ($this->wc_views_check_status_of_layouts_plugin()) {
  				//Layouts activated
  				global $post;
  				if ((isset($post->ID)) && (isset($post->post_type))) {
  					$id_check=$post->ID;
  					$id_check=intval($id_check);
  					$post_type_check=$post->post_type;
  					if (('dd_layouts' == $post_type_check) && ($id_check > 0)) { 
  						if (class_exists('WPDD_layout_post_loop_cell_manager')) {
  							if ((method_exists('WPDD_layout_post_loop_cell_manager','getInstance')) &&
  							(method_exists('WPDD_layout_post_loop_cell_manager','get_options_general'))) {
  							$layouts_archive_object=WPDD_layout_post_loop_cell_manager::getInstance();  							
  								$layout_general_loop_options=$layouts_archive_object->get_options_general();
  								if (is_array($layout_general_loop_options)) {
  									//Here we want to know if this Layout ID being loaded is used in loops
  									//Loops like products, categories or tags.
  									if (in_array($id_check,$layout_general_loop_options)) {
  										//Yes, it is used indeed.
  										$run_filter=true;
  									}
  								}  								
  							}
  		...
  		if ($run_filter) {
  			//We are using these for loops
  			$wcviews_final_shortcodes=$this->wcviews_final_shortcodes;
  			
  			if ((is_array($the_items)) && (isset($the_items['WooCommerce']))) {
  				$woocommerce_candidate_shortcodes=$the_items['WooCommerce'];
  				foreach ($woocommerce_candidate_shortcodes as $k=>$v) {
  					$shortcode_mnenomic=$v[1];
  					if (isset($wcviews_final_shortcodes[$shortcode_mnenomic]['loop_use'])) {
  						$archive_allowed=$wcviews_final_shortcodes[$shortcode_mnenomic]['loop_use'];
  						if (false === $archive_allowed) {
  							unset($the_items['WooCommerce'][$k]);
  						}
  					}
  				}
  			}
  	...
  	public function wcviews_plugin_plugin_row_meta( $plugin_meta, $plugin_file, $plugin_data, $status ) {
  		...
  		if ( ( defined('WOOCOMMERCE_VIEWS_PLUGIN_PATH') ) && ( defined('WC_VIEWS_VERSION') ) ) {
  	...
  	protected function wcviews_execute_woocommerce_before_single_product() {
  		...
  		$is_activated = $this->is_woothumbs_plugin_activated();  		
  		if ( true === $is_activated ) {
  			//	WooThumbs plugin activated
  			//Execute needed hooks for compatibility
  			ob_start();
  			do_action( 'woocommerce_before_single_product' );
  			return ob_get_clean();   			
  		}
