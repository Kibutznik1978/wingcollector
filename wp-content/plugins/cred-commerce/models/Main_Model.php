...
final class CRED_Commerce_Main_Model implements CREDC_Singleton {
    ...
    public function __construct() {
        ...
        if (class_exists('CRED_Loader', false)) {
            ...
            $this->credusermodel = CRED_Loader::get('MODEL/UserForms');
        }
    }
    ...
    public function getForm($form_id, $full = true) {
        ...
        $thismodel = $is_user_form ? $this->credusermodel : $this->credmodel;

        if ($full) {
            $form = $thismodel->getForm($form_id, array('commerce'));
            ...
            $form = new stdClass;
            $form->ID = $form_id;
            $form->commerce = $thismodel->getFormCustomField($form_id, 'commerce');
            //Change this in CRED if possible
            if ($is_user_form) {
                if (isset($form->commerce['order_processing']['post_status']) && empty($form->commerce['order_processing']['post_status'])) {
                    $form->commerce['order_processing']['post_status'] = 'draft';
                }
            }
        }
