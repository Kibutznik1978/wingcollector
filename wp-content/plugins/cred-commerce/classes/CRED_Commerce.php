...
final class CRED_Commerce {
    ...
    public static function cred_delete_draft_users() {
        ...
        $done = __("All draft users have been deleted successful", 'wp-cred');
    ...
    public static function cred_commerce_display_notice() {
        ?>
        <div class="error">
            <p><?php _e('CRED Commerce plugin needs the <a href="https://wp-types.com/home/toolset-components/#cred" target="_blank"><strong>CRED Frontend Editor</strong></a> plugin to be installed and activated.', 'wp-cred-pay'); ?></p>
        </div>
        <?php
    }
    ...
    public static function addUserMetaBoxes($form) {
        global $pagenow;
        // cred commerce meta box
        add_meta_box('credcommercediv', __('CRED Commerce', 'wp-cred'), array(__CLASS__, 'addUserCommerceMetaBox'), null, 'normal', 'high', array());
    }
    ...
    public static function extraNotificationCodes($codes, $form_id, $post_id) {
        cred_log("extraNotificationCodes");
        cred_log($codes);
        cred_log($form_id);
        cred_log($post_id);
...
//                $customer = self::$handler->getCustomer($form_id);
//                cred_log("-------->");
//                cred_log($customer);
//                
//                $customer = get_user_by("id", $post_id);
//                if ($customer) {
//                    $codes['%%CUSTOMER_ID%%'] = $customer->ID;
//                    $codes['%%CUSTOMER_EMAIL%%'] = $customer->billing_email;
//                    $codes['%%CUSTOMER_DISPLAYNAME%%'] = $customer->billing_first_name . " " . $customer->billing_last_name;
//                    $codes['%%CUSTOMER_FIRSTNAME%%'] = $customer->billing_first_name;
//                    $codes['%%CUSTOMER_LASTNAME%%'] = $customer->billing_last_name;
//                }
//            } else {
//                $customer = self::$handler->getCustomer($post_id, $form_id);
//                cred_log("-------->");
...
//            }
    ...
    // setup CRED menus in admin, before CRED 2.0
	...
	// setup CRED menus in admin, after CRED 2.0
	public static function toolset_register_menu_pages( $pages ) {
		if (
			isset( $_GET['page' ] )
			&& $_GET['page'] == 'CRED_Commerce'
		) {
			$pages[] = array(
				'slug'			=> 'CRED_Commerce',
				'menu_title'	=> __( 'CRED Commerce', 'wp-cred-pay' ),
				'page_title'	=> __( 'CRED Commerce', 'wp-cred-pay' ),
