...
final class CRED_Woocommerce_Plugin extends CRED_Commerce_Plugin_Base implements CRED_Commerce_Plugin_Interface {
    ...
    public function getCustomer($post_id, $form_id) {
        ...
        if ($orders) {
            ...
            $order_id = isset($order->id) ? $order->id : $order->post->ID;
            ...
            $user = null;
            if ($order) {
                $user_id = $order->user_id ? $order->user_id : ($order->post->post_author ? $order->post->post_author : 0);
...
//                return $user;
            ...
            $customer->ID = (isset($user)) ? $user->ID : "-";
            ...
            if ($order && $order->post->post_author) {
