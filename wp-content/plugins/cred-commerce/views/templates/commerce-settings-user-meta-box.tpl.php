...
$data = array_merge(array(
    ...
    'product_field' => '',
    'clear_cart' => 1,
    'order_processing' => array('post_status' => 'draft'),
...
<div>
    ...
    <?php if (!$ecommerce) { ?>
        ...
        <div class="cred-commerce-message cred-notification cred-error">
            <p>
                ...
                <?php printf(__('To charge payment for this user form, you first have to install an e-commerce plugin and create products. Then, return here to select the product for the form and continue the setup. %s', 'wp-cred-pay'), $productlink); ?>
            </p>
        </div>
    ...
    <?php } elseif (!$products || empty($products)) { ?>
        <div class="cred-commerce-message cred-notification cred-error">
            <p>
                <i class="icon-warning-sign"></i>
                <?php printf(__('To charge payment for this user form, you first have to <a href="%1$s">create products in %2$s</a>. Then, return here to select the product for the form and continue the setup.', 'wp-cred-pay'), $producthref, $commerceplugin); ?>
            </p>
        </div>
    ...
    <div class="cred_commerce_panel">
        ...
        <fieldset class="cred-fieldset">
            <h4>
                <?php _e('User status when the payment status updates', 'wp-cred-pay') ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <select name="_cred_commerce[order_processing][post_status]">
                                    <optgroup label="<?php echo esc_attr(__('Select user status', 'wp-cred-pay')); ?>">
                                        <option value='' disabled selected style='display:none;'><?php _e('Select post status', 'wp-cred-pay'); ?></option>                                
                                        <option value='draft'<?php if (isset($data['order_processing']['post_status']) && 'draft' == $data['order_processing']['post_status']) echo 'selected="selected"'; ?>><?php _e('Hide user', 'wp-cred-pay') ?></option>
                                        <option value='publish'<?php if (isset($data['order_processing']['post_status']) && 'publish' == $data['order_processing']['post_status']) echo 'selected="selected"'; ?>><?php _e('Create user', 'wp-cred-pay') ?></option>
                                    </optgroup>
                                </select>
                            </td>
                        </tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="_cred_commerce[order_completed][post_status]">
                                <optgroup label="<?php echo esc_attr(__('Select user status', 'wp-cred-pay')); ?>">
                                    <option value='' disabled selected style='display:none;'><?php _e('Select post status', 'wp-cred-pay'); ?></option>
                                    <option value='draft'<?php if (isset($data['order_completed']['post_status']) && 'draft' == $data['order_completed']['post_status']) echo 'selected="selected"'; ?>><?php _e('Hide user', 'wp-cred-pay') ?></option>
                                    <option value='publish'<?php if (isset($data['order_completed']['post_status']) && 'publish' == $data['order_completed']['post_status']) echo 'selected="selected"'; ?>><?php _e('Create user', 'wp-cred-pay') ?></option>
                                </optgroup>
                            </select>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <select name="_cred_commerce[order_refunded][post_status]">
                                <optgroup label="<?php echo esc_attr(__('Select user status', 'wp-cred-pay')); ?>">
                                    <option value='' disabled selected style='display:none;'><?php _e('Select post status', 'wp-cred-pay'); ?></option>                                
                                    <option value='delete' <?php if (isset($data['order_refunded']['post_status']) && 'delete' == $data['order_refunded']['post_status']) echo 'selected="selected"'; ?>><?php _e('Delete user', 'wp-cred-pay') ?></option>
                                    <option value='draft'<?php if (isset($data['order_refunded']['post_status']) && 'draft' == $data['order_refunded']['post_status']) echo 'selected="selected"'; ?>><?php _e('Hide user', 'wp-cred-pay') ?></option>
                                    <option value='publish'<?php if (isset($data['order_refunded']['post_status']) && 'publish' == $data['order_refunded']['post_status']) echo 'selected="selected"'; ?>><?php _e('Create user', 'wp-cred-pay') ?></option>
                                </optgroup>
                            </select>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <select name="_cred_commerce[order_cancelled][post_status]">
                                <optgroup label="<?php echo esc_attr(__('Select user status', 'wp-cred-pay')); ?>">
                                    <option value='' disabled selected style='display:none;'><?php _e('Select post status', 'wp-cred-pay'); ?></option>
                                    <option value='delete' <?php if (isset($data['order_cancelled']['post_status']) && 'delete' == $data['order_cancelled']['post_status']) echo 'selected="selected"'; ?>><?php _e('Delete user', 'wp-cred-pay') ?></option>
                                    <option value='draft'<?php if (isset($data['order_cancelled']['post_status']) && 'draft' == $data['order_cancelled']['post_status']) echo 'selected="selected"'; ?>><?php _e('Hide user', 'wp-cred-pay') ?></option>
                                    <option value='publish'<?php if (isset($data['order_cancelled']['post_status']) && 'publish' == $data['order_cancelled']['post_status']) echo 'selected="selected"'; ?>><?php _e('Create user', 'wp-cred-pay') ?></option>
                                </optgroup>
                            </select>
                        </td>
                    </tr>
...
<div style="display:none">
    <?php if (false) { ?>
        <div id="post_author_tip">
            ...
            <p><?php _e('The e-commerce plugin will create a new user after payment completes. Choose if you want to make that user the author of the submitted user.', 'wp-cred-pay'); ?></p>
        </div>
    <?php } ?>
    <div id="post_status_tip">
        <h3><?php _e('User status', 'wp-cred-pay'); ?></h3>
        <p><?php _e('CRED Commerce can update the status of the user, created by the form, when the payment status changes. This will let you automatically hide, create and delete users when payments complete or are cancelled.', 'wp-cred-pay'); ?></p>
    </div>
    <div id="thankyou_message_tip">
        <h3><?php _e('Message on thank-you page', 'wp-cred-pay'); ?></h3>
        <p><?php _e('This field allows you to add a message to the thank-you page, which clients will see once their payment is complete. In this message, you can describe what your clients should expect next. Use shortcodes to link to the newly-create user or other personalized information. CRED shortcodes', 'wp-cred-pay'); ?></p>
    </div>
            ...
            function replaceFormSubmitEventOption()
            {
                if ($credCommerceEnable.is(':checked'))
                {
                    $_post.find('.cred-notification-event-fieldset').each(function () {
                        ...
                        if (order_modified && order_modified[0] && typeof (order_modified[0].__credCommercePrevValue) != 'undefined')
                    ...
                    $_post.find('.cred-notification-recipient-fieldset').each(function () {
                        ...
                        if (to_customer && to_customer[0] && typeof (to_customer[0].__credCommercePrevValue) != 'undefined')
                        {
